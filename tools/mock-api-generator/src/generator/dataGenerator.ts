...
function generateValue(property: ParsedProperty): unknown {
  ...
  if (nameLower === 'name' || nameLower === 'fullname' || nameLower === 'full_name') return faker.person.fullName();
  ...
  if (nameLower === 'phone' || nameLower === 'phonenumber' || nameLower === 'phone_number') return faker.phone.number();
  ...
  if (nameLower === 'zipcode' || nameLower === 'zip' || nameLower === 'postcode') return faker.location.zipCode();
  if (nameLower === 'url' || nameLower === 'website') return faker.internet.url();
  if (nameLower === 'avatar' || nameLower === 'image' || nameLower === 'photo') return faker.image.avatar();
  if (nameLower === 'description' || nameLower === 'bio' || nameLower === 'summary') return faker.lorem.sentence();
  if (nameLower === 'title') return faker.lorem.words(3);
  if (nameLower === 'price' || nameLower === 'amount' || nameLower === 'cost') return parseFloat(faker.commerce.price());
  if (nameLower === 'age') return faker.number.int({ min: 18, max: 80 });
  if (nameLower === 'createdat' || nameLower === 'created_at' || nameLower === 'updatedat' || nameLower === 'updated_at') {
    return faker.date.past().toISOString();
  }
  if (nameLower === 'active' || nameLower === 'enabled' || nameLower === 'verified') return faker.datatype.boolean();
